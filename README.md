...
- Storage spans under the operation span, SQLite engine, for every storage-backed operation: writes
  (`insert`, `update`, `upsert`, `upsert_by_key`, `delete`), both searches, plain reads (`read_rows`,
  `query`), schema lifecycle (`create_table`, `drop_table`, `create_namespace`, `drop_namespace`),
  `migrate` and `vacuum`. The span is `<db.operation.name> <table>` (`INSERT docs`, `SELECT notes`,
  ...
  reads (`list_*`, `describe_table`, `tokenize`) and the long-lived change feed (`changes_since`,
  `wait_for`, `subscribe`) have no storage span.
