...
func run() error {
	...
	st, err := openStore(cfg, tel.TracerProvider, tel.MeterProvider)
	if err != nil {
		return err
	}
	storeOpen := true
...
func openStore(cfg *config, tp trace.TracerProvider, mp metric.MeterProvider) (store.Engine, error) {
...
func printEnvHelp(out io.Writer) {
	help := []envHelp{
		...
		{"OTEL_LOGS_EXPORTER", "otlp also sends logs to the collector (off unless set; stderr always gets them)"},
		{"OTEL_EXPORTER_OTLP_LOGS_ENDPOINT", "logs-only OTLP endpoint"},
		...
		{"OTEL_SDK_DISABLED", "true turns traces, metrics and log export off"},
