...
- `secret` fields read back as the fixed mask `"••••"` (or `null` when unset) in every read: `read_rows`, both searches, and `query`. To get the plaintext, name the field in `reveal` on `read_rows`, `search_fulltext` or `search_vector` (`"reveal": ["api_token"]`); `query` never reveals. SQL (`query` and search `filter`s) sees only ciphertext, so never filter or join on a secret field. Under `-auth on` reveal is refused for now. Writing a secret needs the server's secret key; without it the write is refused.
