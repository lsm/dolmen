...
func stdioEnv(extra ...string) []string {
	...
	out = append(out, "DOLMEN_EMBED_PROVIDER=none")
...
func startStdio(t *testing.T, args ...string) *stdioProc {
	t.Helper()
	return startStdioWithEnv(t, nil, args...)
}
...
func TestStdioIgnoresHTTPOnlySubscriptionAge(t *testing.T) {
	p := startStdioWithEnv(t, []string{"DOLMEN_MAX_SUBSCRIPTION_AGE=500ms"})
	res := p.rpc("age-init", "initialize", stdioInitializeParams())
	if res["result"] == nil {
		t.Fatalf("dolmen mcp must start with an HTTP-only DOLMEN_MAX_SUBSCRIPTION_AGE value it never uses; stderr: %s", p.stderrText())
	}
	ping := p.rpc("age-ping", "ping", map[string]any{})
	if ping["result"] == nil {
		t.Fatalf("ping after startup failed: %v; stderr: %s", ping, p.stderrText())
	}
}
...
type stdioParityStep struct {
	name string
	op   string
	body map[string]any
}

func stdioParityScript() []stdioParityStep {
	ns := "stdio-par"
	fields := []map[string]any{
		{"name": "title", "type": "string", "fulltext": true},
		{"name": "tag", "type": "string"},
	...
	return []stdioParityStep{
		{"create_namespace", "create_namespace", map[string]any{"namespace": ns}},
		{"create_table", "create_table", map[string]any{"namespace": ns, "table": "docs", "fields": fields}},
		...
		{"describe_table", "describe_table", map[string]any{"namespace": ns, "table": "docs"}},
		{"list_tables", "list_tables", map[string]any{"namespace": ns}},
		{"infer_schema", "infer_schema", map[string]any{
			"samples": []map[string]any{{"a": 1, "b": "x", "c": true}},
		}},
		{"list_migrations", "list_migrations", map[string]any{"namespace": ns, "table": "docs"}},
		{"capabilities", "capabilities", map[string]any{}},
		{"describe_server", "describe_server", map[string]any{}},
		{"list_namespaces", "list_namespaces", map[string]any{}},
