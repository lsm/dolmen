...
// TestSubscribeServerShutdownEndsStream: the server-shutdown signal ends a
// healthy stream — http.Server.Shutdown waits for in-flight handlers but
// never cancels their request contexts, so the stream must end itself when
// Shutdown fires, or every graceful restart would wait out its deadline on a
// connected subscriber (main wires api.Server.Shutdown through
// RegisterOnShutdown). The terminal is the same reconnect teaching the
// engine's closes carry: resume from the last delivered cursor.
func TestSubscribeServerShutdownEndsStream(t *testing.T) {
	h := newHarness(t)
	h.seedTable("rt", "notes", []map[string]any{{"name": "title", "type": "string"}})
	inserted := h.mustHTTP("insert", map[string]any{
		"namespace": "rt", "table": "notes", "records": []any{map[string]any{"title": "x"}},
	...
	// The subscriber stays connected and healthy; only the server stops.
	h.api.Shutdown()

	f := s.next("the shutdown close frame")
	if f.event != "close" {
		t.Fatalf("frame after shutdown = %q event (%s), want close", f.event, f.data)
	}
	closeData := frameData(t, f)
	reason, _ := closeData["reason"].(string)
	for _, teach := range []string{"shutting down", "reconnect"} {
		if !strings.Contains(reason, teach) {
			t.Fatalf("close reason %q does not teach the reconnect path (%q missing)", reason, teach)
		}
	}
	if cursor, _ := closeData["cursor"].(string); cursor == "" {
		t.Fatalf("close frame carries no resume cursor: %v", closeData)
	}
	s.waitEnded("the shutdown close frame")
}

// TestSubscribeShutdownInterruptsReplay: shutdown is observed DURING the
// replay, not only after it — the signal closes no request context, so a
// big backlog (or a slow reader whose writes block) would otherwise page
// straight past main's shutdown deadline before the live select ever ran.
// Kernel buffering decides whether the buffered replay completes first, so
// the fixture pins the ending — the teaching close frame, terminal-last —
// not the delivered-frame count.
func TestSubscribeShutdownInterruptsReplay(t *testing.T) {
	h := newHarness(t)
	h.seedTable("rt", "notes", []map[string]any{{"name": "title", "type": "string"}})
	insertBulk(t, h, 3000) // three pages of backlog

	s := h.subscribeLive(t, url.Values{"namespace": {"rt"}, "cursor": {"begin"}})
	sseChangeOf(t, s.next("the first replayed change"))

	h.api.Shutdown() // mid-replay: later pages may still be in flight

	// Drain whatever the handler had already paged into the socket buffers
	// before the shutdown landed (kernel buffering decides how much) until
	// the stream's last word: the teaching close frame, terminal-last.
	...
	for {
		...
		if f.event != "change" {
			t.Fatalf("frame after mid-replay shutdown = %q event (%s), want change or close", f.event, f.data)
		}
	}
	if reason, _ := closeData["reason"].(string); !strings.Contains(reason, "shutting down") {
		t.Fatalf("close reason %q does not teach the shutdown", reason)
	}
	if cursor, _ := closeData["cursor"].(string); cursor == "" {
		t.Fatalf("close frame carries no resume cursor: %v", closeData)
	}
	s.waitEnded("the mid-replay shutdown close frame")
}

// TestSubscribeEarlyShutdownKeepsRegistrationCursor: a cursor-less stream
// ended before its first page still teaches the exact resume — the engine's
// registration cursor — never an empty cursor that would restart the
// reconnecting client at the newer head and skip the in-flight commits
// landing between registration and the shutdown.
func TestSubscribeEarlyShutdownKeepsRegistrationCursor(t *testing.T) {
	h := newHarness(t)
	h.seedTable("rt", "notes", []map[string]any{{"name": "title", "type": "string"}})

	// A bare start: registration fixes the boundary at the current head.
	s := h.subscribeLive(t, url.Values{"namespace": {"rt"}})
	// Commits land after registration; shutdown preempts their delivery.
	h.mustHTTP("insert", map[string]any{
		"namespace": "rt", "table": "notes", "records": []any{map[string]any{"title": "x"}},
	})
	h.api.Shutdown()
	...
	var closeData map[string]any
	sawChange := false // whether the drainer flushed the in-flight insert before the terminal
	for {
		f := s.next("the shutdown close frame")
		if f.event == "close" {
			closeData = frameData(t, f)
		...
		if f.event != "change" {
			t.Fatalf("frame after shutdown = %q event (%s), want change or close", f.event, f.data)
		}
		sawChange = true
	}
	cursor, _ := closeData["cursor"].(string)
	if cursor == "" {
		t.Fatalf("close frame carries no resume cursor — an early shutdown must teach the registration boundary, not a head start: %v", closeData)
	}
	// The cursor is real and exact: reading from it yields precisely the
	// commits the stream never delivered — the in-flight insert when its
	// frame did not make it out, nothing when it did (the terminal then
	// carries the delivered record's cursor). Either way the resume skips
	// nothing and repeats nothing.
	data := h.mustHTTP("changes_since", map[string]any{"namespace": "rt", "cursor": cursor})
	got := changesOf(t, data)
	wantCount := 1
	if sawChange {
		wantCount = 0
	}
	if len(got) != wantCount {
		t.Fatalf("changes_since from the close cursor = %d changes (sawChange=%v), want %d — the resume must skip nothing and repeat nothing", len(got), sawChange, wantCount)
	}
	s.waitEnded("the early-shutdown close frame")
}
