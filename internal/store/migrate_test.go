...
func TestMigrateVectorizeBackfill(t *testing.T) {
	...
	rows, _, err := st.SearchVector(ctx, "test", "plain", "", qv[0], "fake-space", 0, 1, false, "", nil, nil)
	if err != nil {
...
func TestMigrateVectorizeSwitch(t *testing.T) {
	...
	rows, _, err := st.SearchVector(ctx, "test", "switch", "", qv[0], "fake-space", 0, 2, false, "", nil, nil)
	if err != nil {
...
func TestDropAndReAddVectorizeField(t *testing.T) {
	...
	rows, _, err := st.SearchVector(ctx, "test", "recyc", "", qv[0], "fake-space", 0, 5, false, "", nil, nil)
	if err != nil {
...
func TestEmbedModelMismatchGuard(t *testing.T) {
	...
	if _, _, err := st.SearchVector(ctx, "test", "mm", "", qv[0], "other-space", 0, 5, false, "", nil, nil); err == nil {
...
func TestChunkedVectorizeBackfill(t *testing.T) {
	...
	rows, _, err := st.SearchVector(ctx, "test", "chunky", "", qv[0], "fake-space", 0, 1, false, "", nil, nil)
	if err != nil {
		t.Fatalf("search: %v", err)
	}
...
func TestUnrelatedMigrationPreservesEmbedDim(t *testing.T) {
	...
	if _, _, err := st.SearchVector(ctx, "test", "dimkeep", "", []float32{1, 0, 0, 0}, "", 0, 5, false, "", nil, nil); err == nil {
...
func TestNoOpVectorizeMigrationSkipsReembed(t *testing.T) {
	...
	rows, _, err := st.SearchVector(context.Background(), "test", "noop", "", qv[0], "fake-space", 0, 1, false, "", nil, nil)
...
func TestMigrateReDerivesEmbedDimAfterDisable(t *testing.T) {
	...
	if _, _, err := st.SearchVector(ctx, "test", "redim", "", qv[0], "fake-space", 0, 5, false, "", nil, nil); err != nil {
...
func TestPlanEstimatesUsePreMigrationNamesForRenamedVectorField(t *testing.T) {
	...
	rows, _, err := st.SearchVector(ctx, "test", "renvec", "", qv[0], "fake-space", 0, 5, false, "", nil, nil)
...
func TestRenameCycleEstimateTerminates(t *testing.T) {
	...
	rows, _, err := st.SearchVector(ctx, "test", "cycle", "", qv[0], "fake-space", 0, 5, false, "", nil, nil)
...
func TestVacatedNameReuseEstimateTracksFieldIdentity(t *testing.T) {
	...
	qv, _ := fakeEmbed(ctx, []string{"two"})
	rows, _, err := st.SearchVector(ctx, "test", "vacate", "", qv[0], "fake-space", 0, 5, false, "", nil, nil)
	if err != nil || len(rows) < 1 || rows[0]["b"] != "two" {
