...
func TestTypedReadRoundTripSearchVector(t *testing.T) {
	...
	res, err := st.SearchVector(ctx, "test", "typed", "vec", []float32{1, 0, 0, 0}, "", 0, 10, false, "", nil, nil)
	if err != nil {
...
func TestSearchHidesEmbeddingByDefault(t *testing.T) {
	...
	vecRes, err := st.SearchVector(ctx, "test", "notes", "", qv[0], "fake-space", 0, 1, false, "", nil, nil)
	if err != nil {
		t.Fatalf("vector search: %v", err)
	}
	...
	vecRes, err = st.SearchVector(ctx, "test", "notes", "", qv[0], "fake-space", 0, 1, true, "", nil, nil)
	if err != nil {
