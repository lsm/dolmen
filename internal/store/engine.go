...
// explicitly null, never omitted. Notifications and subscribe turned TRUE
// with 6b's landing: Listen is implemented and the /v1/subscribe stream is
// registered, so the capability answers "is Listen implemented?" honestly
// in both directions — a conforming engine must never report a capability
// it does not have, whatever the cost of saying false.
