...
func TestMigrateUnknownChangeKeyErrors(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			want:     `changes[0]: unknown migration op "add_filed" (valid: add_field, rename_field, drop_field, set_fulltext, set_vectorize, set_enum, set_shape)`,
			wantHint: false,
		},
		{
			name:     "missing op",
			changes:  []map[string]any{{"field": map[string]any{"name": "status", "type": "string"}}},
			want:     `changes[0]: op must be a non-empty string naming the change (add_field, rename_field, drop_field, set_fulltext, set_vectorize, set_enum, set_shape)`,
			wantHint: false,
		},
		{
			name:     "non-string op",
