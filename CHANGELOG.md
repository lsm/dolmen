...
- **Per-row ownership.** A table declared with `row_access: "own"` carries an implicit `owner`
  ...
  follow it too: a scoped caller may name a table and catch up on its own rows, foreign commits
  never wake it, and the namespace-wide feed still needs `read` on the namespace because it reports
  every table. Cursor tokens are opaque random values, so a scoped reader's sequence cannot be told
  from one where the foreign commits never happened, and retention expires by age rather than by
  volume, so other principals' traffic cannot evict a quiet reader's cursor. Scoped
