...
engine's unauthenticated `filter` is its own, and may be narrower.
...
| D32 | **The unauthenticated `filter` language is the engine's own** (2026-09-21, resolving a contradiction between §4.3 and §4.4). §4.4 claimed scoped `filter`/`args` "stay portable by construction on every engine", but §4.3's allowlist binds only under `auth: on`, leaving `auth: off`'s `filter` unconstrained — and it diverges in practice: PostgreSQL compiles a filter as a confined single-table select, so a cross-table subquery SQLite accepts answers `not_found` there. Two readings were possible. Applying the allowlist under `auth: off` too would make the portability claim true everywhere, but it breaks §8.1 outright — a v0.2.0-valid subquery filter would start failing on adapter #1, which is exactly the regression §8.1 forbids. So: **§8.1's byte-for-byte promise binds the engine v0.2.0 shipped on**; it is a non-regression guarantee for a deployment that upgrades, not a portability mandate over engines that did not exist when it was written. Another engine's unauthenticated `filter` may be narrower, and says so through the same disclosure obligation as `query` (§4.4). Under `auth: on` the engines converge, because §4.3's allowlist forbids cross-table references on all of them — the divergence exists only in the unauthenticated language, and only in the direction of being stricter. Conformance pins both halves in one fixture rather than skipping either. | §4.3, §4.4, §8.1 |
