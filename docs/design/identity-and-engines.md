...
| Ranking quality | **Amended 2026-09-17.** The normative full-text ranking is **dolmen's own BM25, implemented once in shared Go** and used by every engine, with parameters k1=1.2, b=0.75, all column weights 1.0, computed over the visible corpus. **Sign convention, pinned here because FTS5 no longer supplies it:** the shared scorer emits FTS5-convention ranks — more relevant is *lower* (more negative) — so the Ordering row's `q(rank)` ASCENDING stays relevance-descending on every engine and in both tiers. The value itself is never exposed (see Result shape), so the convention is internal; it is pinned only so ascending ordering is well defined without reference to FTS5. "BM25-family" is not a license for a variant: differing idf definitions, length normalization, parameters, or field weights reorder the same corpus while still feeling like BM25 — but the variant that matters is pinned by the shared implementation itself, not by an external oracle. Identical corpus + query ⇒ identical order on every engine **by construction**, because there is only one scorer; an engine supplies token streams and postings, never a score. **SQLite FTS5 is no longer the reference oracle** (superseding the pre-amendment rule that other engines reproduce adapter #1's emitted ranks bit-for-bit): FTS5 remains a storage and matching accelerator, and its output is a useful quality comparison in tests, but it is not normative and an engine is not required to reproduce it. Near-equal scores still compare with the same canonical quantization as vectors, `q(rank) = floor(rank / fl64(1e-9))` with `id` tiebreak, so precision and evaluation order cannot reorder documents or shift offset pages between engines. **Open — the `auth: off` tier (§8.1):** adapter #1 today orders `ORDER BY rank, rowid` using FTS5's own ranks, and §8.1 promises that mode is preserved bit-for-bit. Adopting the shared scorer there would change the order existing v0.2.0 callers see. Either SQLite keeps FTS5 ranking under `auth: off` and uses the shared scorer elsewhere (two paths on one engine, §8.1 intact), or the shared scorer applies everywhere and §8.1's full-text ordering guarantee is explicitly relaxed. Not decided here; the extraction slice must not land before it is. |
...
| D27 | Full-text ranking is **dolmen's own BM25, implemented once in shared Go** and used by every engine (k1=1.2, b=0.75, weights 1.0, over the visible corpus); engines supply token streams and postings, never a score, so identical corpus + query ⇒ identical order by construction. **Supersedes the FTS5-as-reference-oracle rule** — matching SQLite FTS5's emitted ranks is explicitly not required (FTS5 stays a storage/matching accelerator and a test-time quality comparison). Ranks keep FTS5's sign convention (lower = more relevant) so `q(rank)` ASC stays relevance-descending; the value is never exposed. **Open:** whether `auth: off` adopts the shared scorer (relaxing §8.1's full-text ordering guarantee) or keeps FTS5 ranking (two paths on adapter #1) — the extraction slice is blocked on it | §7 |
