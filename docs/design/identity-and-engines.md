...
evaluator across engines. Confinement is unchanged and non-negotiable: §0.5.3's catalog and function
...
| D28 | `query` is **transparent**: caller SQL reaches the engine as written — no translation layer, no portable subset (a partial SQL translator fails silently on what it does not cover). The obligation is **disclosure, not portability**: each engine states its dialect on the capability surface so clients branch on it rather than meet a syntax error. §4.3's filter allowlist remains the portable lane for `filter`/`args`, and §0.5.3 catalog/function confinement applies to every engine exposing `query` | §7 |
