...
| D31 | Disabling `row_access` (§4.2) does not re-open the §2 alternative verb path: with the annotation off, a data-verb holder without `read` is refused rather than promoted to table-wide reach (2026-09-20). §4.2's "widens every data-verb holder's reach" describes the hazard the `admin` requirement guards, not a grant of new visibility to callers holding no `read`; §2's "or any data verb **when the table declares `row_access`**" is literal and fail-closed | §2, §4.2 |
