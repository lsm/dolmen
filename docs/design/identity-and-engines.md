...
type Engine interface {
    ...
    //   query_dialect:    string                   (added 2026-09-17 with D28: the
                                               SQL dialect `query` and search
                                               `filter` accept, since caller SQL is
                                               passed through untranslated. Lowercase
...
engine chose for itself. Adapter #1 reports `"sqlite"`. The value is an open enum — a new engine
...
| D28 | `query` is **transparent**: caller SQL reaches the engine as written — no translation layer, no portable subset (a partial SQL translator fails silently on what it does not cover). The obligation is **disclosure, not portability**: each engine states its dialect in the pinned `query_dialect` capability field (string, open enum — `"sqlite"` on adapter #1; §6, §2) so conformance can compare it and clients branch on it rather than meet a syntax error. §4.3's filter allowlist remains the portable lane for `filter`/`args`, and §0.5.3 catalog/function confinement applies to every engine exposing `query` | §7 |
