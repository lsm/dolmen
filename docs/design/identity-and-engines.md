...
| `-trusted-proxies` | `DOLMEN_TRUSTED_PROXIES` | empty | Comma-separated CIDRs (bare IPs allowed). Only peers inside these ranges may assert §1.1 headers. The same peers are the only ones whose forwarding headers (`X-Forwarded-Host`/`-Proto`/`-Prefix`, `Forwarded`, original-URI headers) shape the public URLs dolmen advertises; from other peers those headers are dropped before routing, in every auth mode. *Why one list (amended 2026-09-23, #328): both answer "which peer is my proxy"; a proxy listed only for its URL must strip client-sent §1.1 headers.* |
...
| `capabilities` | none (any authenticated principal; unauthenticated under `auth: off`) | Untargeted: reports the engine capability surface from `Engine.Capabilities()` (§6) — `vector_execution` (`"exact"` \| `"ann"`), `ann_recall_bound` (explicit `null` in exact mode, never omitted; iff ann a number in (0,1] — the engine's **guaranteed minimum Recall@10 versus the exact path over the conformance corpus**, a worst-case bound, never an empirical average), `notifications` (bool), `subscribe` (bool), `query_dialect` (string — the SQL dialect `query` accepts, e.g. `"sqlite"` on adapter #1; **not** scoped `filter`/`args`, which stay §4.3's shared allowlist on every engine; open enum per D28, added 2026-09-17), `filter_dialect` (string — the dialect an **unauthenticated** `filter` is read in, named the same way; under `auth: on` §4.3's allowlist binds every engine and the field is informational; open enum per D32, added 2026-09-21); field names and types pinned so the discovery is portable and conformance-comparable, enum values closed except the two dialect fields, unknown fields additive (§8.1) — **in both modes** (additive under `auth: off` per §8.1). Realtime ops exist in both modes and an `auth: off` client has no other discovery surface (`describe_server`'s extension is `auth: on`-only); this op is that surface, and it is the single source `describe_server` inlines under `auth: on`. |
...
Authorization is a **direct implementation of §3.3's union**, over a grant table in the registry
below (**amended 2026-09-19, D30**, superseding "embedded OpenFGA (SQLite tuple store, groups
resolved as contextual tuples per request)"). The grant model is an implementation detail behind
the three ops below; it never surfaces in the API, which is why the substitution changes no
request, no response, and no conformance expectation.

*Why not OpenFGA, measured.* Linking it costs **+20.5 MB** (dolmen 20.8 MB → 41.3 MB): its check
engine lives under `internal/`, so `pkg/server` is the only importable entry, and it carries the
gRPC server, grpc-gateway, Prometheus, TOML config, and CEL + antlr for conditional tuples dolmen
does not use — plus a protobuf reflection registry whose generated types defeat dead-code
elimination and dominate `runtime.pclntab` and the type descriptors. §3.3 needs none of that
machinery: the answer is a union over at most (1 principal + N groups) subjects × (`*` + path
ancestors + namespace + table) objects, with no deny grants, no precedence, and no ordering. That
is one indexed lookup over a bounded row set, not a relationship graph. OpenFGA earns its cost
where relationships are recursive and conditional; here it would be 20 MB and a large transitive
dependency surface to union roughly ten rows. If the model ever grows recursive relationships or
conditions, this decision is the thing to revisit.
...
- Enabling `row_access` later via `migrate` (`{"op": "set_row_access", "value": true}`) is rejected
  ...
  **Clarified 2026-09-20 (D31):** "filtering stops" does not widen the *alternative verb path*.
  §2 grants own-row reach through a data verb only *when the table declares `row_access`*, so once
  it is off, a data-verb holder without `read` has no read path at all and is refused — the
  admin requirement below still stands, because the holders who do have `read` go from
  table-wide-by-grant to table-wide-by-default and every stored `owner` value becomes visible to
  them. The two readings are reconciled in favour of the literal §2 rule, which is the fail-closed one. It
...
the filter language is unchanged from v0.2.0 **on the engine v0.2.0 shipped on** — see D32: another
engine's unauthenticated `filter` is its own dialect, neither a subset nor a superset of SQLite's.
...
membership), `set_shape` (shape membership: its rejection counts and names the rows
whose stored value does not fit, so unscoped it would be an oracle over values the caller cannot
read), `set_row_access` enabling (row existence), `add_field` of a required field without a
...
type Engine interface {
    ...
    //   filter_dialect:   string                   (added 2026-09-21 with D32: the
    //                                             dialect an unauthenticated filter is
    //                                             read in; informational under auth: on,
    //                                             where §4.3's allowlist binds. Open enum.)
    //   query_dialect:    string                   (added 2026-09-17 with D28: the
                                               SQL dialect `query` accepts, since
                                               caller SQL is passed through
                                               untranslated. NOT the dialect of
                                               scoped `filter`/`args`, which stay the
                                               shared engine-neutral allowlist of
                                               §4.3 on every engine. Lowercase
                                               engine-family name — "sqlite" on
                                               adapter #1, "postgres" on adapter #2;
                                               open enum, not closed, so a new engine
                                               names itself without a spec edit.
                                               Additive under §8.1, both modes)
    // — unknown future fields are additive (§8.1 rules), enum values are
    // closed, with TWO exceptions: query_dialect is an open enum per D28
    // and filter_dialect is one per D32
    // (its job is to name which dialect, not to constrain the set).
    // The op layer publishes it verbatim, never invents it.
...
**Decision amended 2026-09-19: use each storage engine's native search execution.**
Full-text relevance scores and ordering are engine-specific. SQLite keeps FTS5 BM25;
PostgreSQL uses native `tsvector`/GIN matching and `ts_rank_cd` ranking in PostgreSQL.
There is no shared Go full-text scorer and no cross-engine ranking-parity requirement.
Moving a dataset to another engine may change its relevance order; this is accepted
migration behavior, not a compatibility defect. SQLite's existing ordering is unchanged.

The portable contract is operation/result shape, filtering and authorization, bounded
pagination, truncation, and deterministic row-id tie-breaking within each engine.
Document each engine's text-search configuration, tokenization, stemming, stop words,
and ranking. The shared core query grammar below remains portable, but native text
analysis can produce different matching sets (for example for stop words); do not claim
identical linguistic behavior. Unsupported extended syntax returns a teaching error.
An index change within an engine must preserve that engine's documented semantics.
Exact vector score semantics remain as specified below; this amendment resolves
full-text search and does not silently change the separate vector/ANN contract.

**One exception, `search_vector` only (amended 2026-09-06):** an engine MAY
...
The SQL dialect of `query` is SQLite SQL on adapter #1; engine-2's
dialect stance is engine-2's issue, and must keep the conformance corpus green. ~~The SQL dialect of
`filter`/`args` on searches~~ — **corrected 2026-09-17:** scoped `filter`/`args` are NOT
engine-dialected. §4.3 pins them to the engine-neutral allowlist evaluated with one shared
evaluator on every engine, which is exactly why D28 can make `query` transparent without stranding
callers who need portability.

**Decided 2026-09-17: `query` is transparent, and the dialect is engine-documented.** Caller SQL is
passed to the engine as written — no translation layer, no portable subset for `query`. SQL is too
large a language to translate honestly, and a partial translator is worse than none because it fails
silently on the cases it does not cover. The obligation this creates is **disclosure, not
portability**: an engine states which dialect it speaks on its capability surface so a client can
branch on it rather than discover the difference from a syntax error. The portable lane already
exists for callers who need one — §4.3's filter allowlist on `filter`/`args`, which stays a shared
evaluator across engines **under `auth: on`** (see D32 for what `auth: off` promises). The disclosure has a **pinned name**, or it is not portable: the capability
surface carries `query_dialect` (string — §6's `Capabilities()` serialization and §2's `capabilities`
row), so conformance can compare it and a client can branch on it instead of guessing from a key an
engine chose for itself. It describes `query` alone; scoped `filter`/`args` stay portable by
construction under §4.3's shared allowlist, on every engine — **scoped meaning `auth: on`**, which is
where the allowlist binds. The unauthenticated `filter` is a different animal and D32 says so. Adapter #1 reports `"sqlite"`. The value is an open enum — a new engine
names its family without a spec edit — because the field's job is to say *which* dialect, not to
constrain the set. Confinement is unchanged and non-negotiable: §0.5.3's catalog and function
rejection applies to every engine exposing `query`, whatever its dialect.
...
| D32 | **The unauthenticated `filter` language is the engine's own dialect** (2026-09-21, corrected the same day; resolving a contradiction between §4.3 and §4.4). §4.4 claimed scoped `filter`/`args` "stay portable by construction on every engine", but §4.3's allowlist binds only under `auth: on`, leaving `auth: off`'s `filter` unconstrained — and it diverges in practice. Adapter #2 compiles a mutation filter through the *same* parser boundary as `query` (`compileSQL`), so its unauthenticated filter inherits `query`'s function and operator sets: it **rejects** what SQLite accepts (a cross-table subquery answers `not_found`, since the compile is confined to one table) and equally **accepts** what SQLite rejects (`now`, `md5`, `regexp_match`, the JSON and regex operators), while shared spellings can mean different things — PostgreSQL's `LIKE` is case-sensitive where SQLite's is ASCII-case-insensitive, so one filter returns different rows on each. It is a dialect, not a narrowing, and the first draft of this decision was wrong to call the divergence one-directional. Applying §4.3's allowlist under `auth: off` too would make the portability claim true everywhere, but it breaks §8.1 outright — a v0.2.0-valid filter would start failing on adapter #1, the regression that section forbids. So: **§8.1's byte-for-byte promise binds the engine v0.2.0 shipped on**, a non-regression guarantee for a deployment that upgrades rather than a portability mandate over engines that did not exist when it was written. Under `auth: on` the engines do converge, because §4.3's allowlist binds all of them and pins the evaluation semantics — that is where the shared-evaluator claim is true, and §4.4's two portability sentences are qualified to it. **Disclosure is paid** (2026-09-21, #386): the capability surface carries `filter_dialect` beside `query_dialect`, both open enums naming a SQL family, so an `auth: off` client branches on the dialect instead of discovering it from an error. Under `auth: on` the field is informational, because §4.3's allowlist binds every engine — a convergence adapter #2 does not yet deliver (#388). | §4.3, §4.4, §8.1 |
| D31 | Disabling `row_access` (§4.2) closes the §2 alternative **verb** path but does not shrink the **visible set** (2026-09-20, corrected 2026-09-21). §2's "or any data verb **when the table declares `row_access`**" is literal and fail-closed, so with the annotation off a holder of data verbs alone is refused the read ops — `403`, not an empty answer. The scope is a separate question and §4.3's first bullet settles it: a table *without* `row_access` gives every `read`-or-data-verb holder **no scope at all**, whatever vestigial `owner` column the migration left behind. The first draft of this decision conflated the two and returned an empty scope, which was invisible while scoped mutations were refused outright and wrong the moment they landed: `update`/`delete` matched nothing and reported zero, and `upsert` took the no-match branch and duplicated a row its filter had matched. That widening of reach is not an accident of the correction — it is exactly what §4.2's `admin` requirement for disabling exists to guard, and a formerly-scoped table must not behave differently from a table that never had the annotation | §2, §4.2, §4.3 |
| D30 | §3's authorization is a **direct implementation of §3.3's union** over a grant table, not embedded OpenFGA (2026-09-19, superseding the §3 preamble). Measured: linking OpenFGA costs +20.5 MB (20.8 → 41.3), almost all of it gRPC/protobuf/CEL and the runtime metadata generated protobuf forces, because the check engine is under `internal/` and `pkg/server` is the only public entry. §3.3 is union-only over a bounded subject × object set — one indexed lookup. FGA was already declared an API-invisible implementation detail, so no op, response, or conformance expectation changes. Revisit if the model ever grows recursive relationships or conditional tuples | §3, §3.3 |
